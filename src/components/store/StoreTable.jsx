...
const { setStoreId, resetStoreId } = StoreActions;
...
const StoreTable = props => {
  ...
  const { history, setStoreId, resetStoreId } = props;
  ...
  const formAction = useCallback(
    (action, data = null) => {
      ...
      if (action === "add") {
        ...
        history.push(`/store/edit/${data.storeId}`, data);
      }
    },
    [history]
  );
      ...
      Cell: props => (
        <div className="react-action-class">
          <Button
            ...
            onClick={() => {
              setviewData(props.original);
              setKeyModal(true);
            }}
          >
            <div className="fs-14 medium-text">
              <i className="fas fa-key mr-6" />
              {props.original?.storeUniqueKey ? "View" : "Generate"}
            </div>
          </Button>
        </div>
      )
      ...
      Cell: props => (
        <div className="react-action-class">
          <Button
            ...
            onClick={() => {
              setStoreId({
                id: props.original.storeId,
                name: props.original.storeName
              });
              history.push("/store/dashboard");
            }}
          >
            <div className="fs-14 medium-text">
              <i className="fas fa-chart-line mr-6" /> Dashboard
            </div>
          </Button>
          <Button
            className="c-btn c-info  mr-10"
          ...
          <Button
            ...
            onClick={() => {
              setToggle(!toggle);
              setdeleteId(props.original.storeId);
            }}
          >
            <div className="fs-14 medium-text">
  ...
  const callListApi = params => {
    setLoader(true);
    Api.post("/admin/store/list", params)
      .then(res => {
  ...
  const handleYes = () => {
    setLoading(true);
    Api.delete(`/admin/store/delete?storeId=${deleteId}`)
      .then(res => {
  ...
  const downloadCSV = async () => {
    ...
    const response = await Api.get("/admin/store/exportCSV");
    const fileName = `stores-${moment().format("MM-DD-YYYY")}.csv`;
        ...
        <div className="roe-card-style mtb-15">
          <div className="roe-card-header module-header">
            <div className="flex-1 fs-16 demi-bold-text">
              <span className="hash"># </span> Store List
            </div>
            <div className="mr-10">
              <input
                value={searchText}
            ...
            <div>
              ...
              >
                <i className="fas fa-plus mr-10"></i>Add Store
              </Button>
              <Button
                className="c-btn ma-5"
                onClick={downloadCSV}
          ...
          <CommonModal
            ...
            viewModalCheck={true}
            headerData={"Store Unique Key"}
            children={
              <GenerateKey
                data={{
                  storeId: viewData?.storeId,
                  storeUniqueKey: viewData?.storeUniqueKey
                }}
              />
            }
          />
...
export default compose(
  connect(mapStateToProps, { setStoreId, resetStoreId }),
  withRouter
)(StoreTable);

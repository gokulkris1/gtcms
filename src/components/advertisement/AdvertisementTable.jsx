...
const AdvertisementTable = ({ isStore = false, ...props }) => {
    ...
    (action, data = null) => {
      ...
      if (action === "add") {
        !isStore
          ...
          : history.push("/store/advertisement/add");
      } else if (action === "edit") {
        history.push(`/advertisement/edit`, data);
        !isStore
          ? history.push(`/advertisement/edit`, data)
          : history.push(`/store/advertisement/edit`, data);
      }
    },
    [history]
  );
  ...
  const callListApi = params => {
    ...
    const url = !isStore
      ...
      : `/admin/advertisement/list?storeId=${storeId}`;
    Api.post(url, params)
      .then(res => {
  ...
  const handleYes = () => {
    setLoading(true);
    Api.delete(`/admin/advertisement/delete?deviceTokenId=${deleteId}`)
      .then(res => {
        ...
        <div className="roe-card-style mtb-15">
          <div className="roe-card-header module-header">
            <div className="flex-1 fs-16 demi-bold-text">
              <span className="hash"># </span>Advertisement List
            </div>
            <div className="mr-10">
              <input
                value={searchText}
            ...
            <div>
              ...
              >
                <i className="fas fa-plus mr-10"></i>Add Advertisement
              </Button>
            </div>
          </div>
          ...
          <CommonModal
            ...
            viewModalCheck={true}
            headerData={"Advertisement Details"}
            size={viewData?.fileType === "TEXT" && "lg"}

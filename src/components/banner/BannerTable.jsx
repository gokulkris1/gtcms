...
const BannerTable = ({ isStore = false, ...props }) => {
    ...
    (action, data = null) => {
      ...
      if (action === "add") {
        !isStore
          ...
          : history.push("/store/banner/add");
      } else if (action === "edit") {
        !isStore
          ? history.push(`/banner/edit`, data)
          : history.push(`/store/banner/edit`, data);
      }
    },
    [history]
  );
  ...
  const callListApi = params => {
    ...
    const url = !isStore
      ? "/admin/banner/list"
      : `/admin/banner/list?storeId=${storeId}`;
    Api.post(url, params)
      .then(res => {
  ...
  const handleYes = () => {
    setLoading(true);
    Api.delete(`/admin/banner/delete?deviceTokenId=${deleteId}`)
      .then(res => {
        ...
        <div className="roe-card-style mtb-15">
          <div className="roe-card-header module-header">
            <div className="flex-1 fs-16 demi-bold-text">
              <span className="hash"># </span>Banner List
            </div>
            <div className="mr-10">
              <input
                value={searchText}
            ...
            <div>
              ...
              >
                <i className="fas fa-plus mr-10"></i>Add Banner
              </Button>
            </div>
          </div>

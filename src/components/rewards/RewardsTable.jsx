...
const RewardsTable = props => {
  ...
  const formAction = useCallback(
    (action, data = null) => {
      ...
      if (action === "add") {
        ...
        history.push(`/rewards/edit/${data.redeemId}`, data);
      }
    },
    [history]
  );
      ...
      Cell: props => (
        <div>
          {props.original.expireDate
            ? moment(props.original.expireDate).format("YYYY-MM-DD")
            : "-"}
        </div>
      )
    },
      ...
      Cell: props => (
        <div className="react-action-class">
          ...
          <Button
            ...
            onClick={() => {
              setToggle(!toggle);
              setdeleteId(props.original.redeemId);
            }}
          >
            <div className="fs-14 medium-text">
  ...
  const callListApi = params => {
    setLoader(true);
    Api.post("/admin/redeem/list", params)
      .then(res => {
  ...
  const handleYes = () => {
    setLoading(true);
    Api.delete(`/admin/redeem/delete?redeemId=${deleteId}`)
      .then(res => {
        ...
        <div className="roe-card-style mtb-15">
          <div className="roe-card-header module-header">
            <div className="flex-1 fs-16 demi-bold-text">
              <span className="hash"># </span> Rewards List
            </div>
            <div className="mr-10">
              <input
                value={searchText}
            ...
            <div>
              ...
              >
                <i className="fas fa-plus mr-10"></i>Add Rewards
              </Button>
            </div>
          </div>
          ...
          <CommonModal
            ...
            size="lg"
            viewModalCheck={true}
            headerData={"Rewards Details"}
            children={<RewardsModal data={viewData} />}
          />
